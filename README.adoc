...
`esel-proc` is a generated SoC powered by RISC-V. Currently `esel-proc` supports the https://github.com/olofk/serv[award-winning serv] and https://github.com/YosysHQ/picorv32[YosysHQ PicoRV32]. The processor executes fetches from a generated ROM that contains the cross-compiled C solution of the latchup problem. The processor communicates with the main latchup streaming interface via a databus peripheral `rtl/eselproc_latchup_interface` (LIF). 
...
[,bash]
----
make [SOL=01_multiplier, 02_divider, ...] [CPU=serv, picorv32]
----
...
[,bash]
----
make test [SOL=01_multiplier,...] [CPU=serv, picorv32]
