...
`esel-proc` is a generated SoC powered by the https://github.com/olofk/serv[award-winning serv] RISC-V processor. SERV executes a program from a generated ROM that contains a cross-compiled C solution of the latchup.app problem. The Soc interfaces with the main latchup streaming interface via a databus peripheral `rtl/eselproc_latchup_interface` (LIF). 
In essence, `esel-proc` emulates the hardware by executing the C solution and Memory-mapped Input/Output (MMIO) to the LIF. (https://github.com/JoGei/latchup-subs[CODE] for reference of my more traditional designs)
